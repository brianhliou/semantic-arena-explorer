...
\author{Brian Liou\\\small Independent Researcher\thanks{Contact: \texttt{brianhliou@gmail.com}. Preprint; comments welcome.}}
\date{\today}
...
separate essentially every subset in 300 dimensions, so the binding constraint
...
WordNet replication reproduces the high-count collapse with no embedding,
bounding when embedding cosine similarity is a safe proxy for communication.
\end{abstract}
...
The one-word channel is much weaker than the game's nominal ceiling, and the
reason is specific. Recovering an arbitrary 9-of-25 subset requires about 21
bits, while one common-English word carries about 12.8, so no \emph{universal}
one-word strategy can exist; the substantive question is per-instance existence.
Under a frozen common-English vocabulary and a deterministic embedding-rank
decoder, exact recovery of a 9-word subset is available for only $0.286\%$ of
board-relative target sets, measured by exact enumeration over $2.0 \times
10^{8}$ assignments, with a practical frontier near six. This limit is not
geometric. Synthetic vectors separate essentially every target subset in 300
dimensions, so the wall is the \emph{projection} from an ideal separating
direction onto a legal word: few vocabulary items realize the direction the
geometry allows. The separations that the decoder does certify then fail a second
test. In a controlled audit with positive controls, large-language-model
receivers recovered ordinary semantic clues at 23 of 24 but recovered
decoder-certified challenge clues, every one a proven exact recovery under the
embedding decoder, at 0 of 44; twenty additional best-available negative-margin
clues also transferred at zero, and a stronger receiver tier reproduces the same
zeros. Decoder-recoverability and natural communicability come apart. The
variable that dominates natural recoverability is not target count but the
availability of a clean separating concept: a single same-category distractor on
the board collapses exact recovery, and count retains only a secondary effect
within clean clusters. The collapse is not an artifact of constructed challenge
sets: on natural first-turn game distributions, exact recovery at counts 4
through 9 was zero across 384 blinded audits, and cluegivers allowed to choose
their own move retreated to counts 1 through 3. A symbolic analysis over WordNet
hypernym concepts reproduces the same shape, which argues against the explanation
being only a GloVe-geometry artifact.

The result matters beyond the game. Embedding cosine similarity is a common proxy
for ``this clue communicates this concept,'' and the dissociation we report shows
the proxy is unsafe at the high-count, low-margin regime where it is most
tempting to trust. Systems that generate clues, captions, or category labels by
optimizing a similarity decoder are optimizing a target that a pragmatic receiver
does not share. The constrained-reference framing also connects the problem to
...
\begin{enumerate}
  ...
  \item An \textbf{exact characterization and measurement} under the
    ...
    binding constraint is vocabulary projection rather than embedding geometry.
...
Section~\ref{sec:related} places the work against Codenames AI benchmarks,
referential and signaling games, rational speech-act pragmatics, and the
discreteness of vocabulary relative to continuous embeddings.
Section~\ref{sec:problem} gives the formal problem and the model ladder.
Section~\ref{sec:bounds} states the theoretical bounds. Section~\ref{sec:setup}
describes the experimental setup. Section~\ref{sec:results} presents results as a
single argument from upper bounds down to pragmatic transfer.
Sections~\ref{sec:discussion} through~\ref{sec:conclusion} discuss, bound, and
conclude.

% =====================================================================
% TODO: convert remaining sections from docs/semantic-subset-paper-v1.md
% Order matches the markdown source. Each \section below is a stub.
% =====================================================================
...
The known-decoder model is a pragmatic speaker that selects an utterance by
modeling the listener. The Rational Speech Acts framework
\citep{frank2012predicting} formalizes the speaker-models-listener recursion,
with the cluegiver as speaker, the clue as utterance, the guesser as listener,
and the target subset as referent. Neural instantiations rerank utterances
against a learned listener model \citep{andreas2016pragmatics}, close to the
simulate-the-partner move available in the known-decoder regime. Signaling-game
theory \citep{lewis1969convention, skyrms2010signals} supplies the language of
conventions and learned sender/receiver mappings, and work on common ground
bounds the shared context a clue may presume. These frameworks describe how a
decoder-aware speaker should choose; they do not quantify, for a fixed board and
vocabulary, which referents such a speaker can reach.
...
The ladder gives the paper its spine. Rungs (1) and (5) are upper bounds that
establish that neither channel capacity nor geometry is the binding constraint.
...
geometry is permissive, which localizes the binding constraint to the vocabulary.
...
The natural / pragmatic model of Section~3.2 has no closed form, so we approximate
...
One construction artifact was found and fixed after the pilot runs: the
hand-authored control items listed their targets as the first \texttt{count} board
words, so the original control boards leaked target position. No challenge or
cluster stratum shares the leak (mean head-of-board target overlap 0.20--0.44, no
leaked items). Rerunning the controls with shuffled boards across all four
receivers reproduces the pass, 46/48 exact with 97.9--100\% target-hit rates, so
control performance reflects meaning rather than position; the control builder now
shuffles board order, and the v0.2 review packet is leak-free in every stratum.
...
\subsection{The Bottleneck Is Vocabulary Projection, Not Geometry}
...
vocabulary deepens the collapse monotonically, which is the signature of a
projection constraint rather than a geometric one (sampled positive-margin rates,
...
perfectly and recovered decoder-certified challenge clues at zero. The challenge
items split by certification status (Section~5.2): the 44 decoder-certified items
...
challenge clues. A clue that maximizes the projection margin is exploiting how a
...
model & controls exact & certified challenge exact & negative-margin exact \\
\midrule
\texttt{gpt-5.4}        & 12/12 & 0/22 & 0/10 \\
\texttt{claude-sonnet-4-6} & 11/12 & 0/22 & 0/10 \\
\bottomrule
\end{tabular}
\end{center}
...
the labeler) was labeled by the first author as a blinded guesser. Controls passed
at 12/12 exact with a 100\% target-hit rate, the study's reviewer-validity gate.
...
clues recovered 0 of 24. The cluster strata land almost exactly on the LLM rates:
...
The author-pilot controls used the original leaked-position boards
(Section~5.2), so the labeler's 12/12 there is not by itself diagnostic; the
labeler's semantic competence is independently evidenced by the unleaked cluster
stratum at 85.1\% target hits. One hypothesis-aware author is not an independent
...
The results compose into a layered account of where one-clue communication breaks.
Channel capacity is permissive: the codebook bound is finite, and
Proposition~\ref{prop:pigeonhole} rules out only a universal one-word protocol
(6.1). Geometry is permissive too: in 300 dimensions the board vectors lie in
general position, so a separating direction exists for essentially every target
subset (6.1, Proposition~\ref{prop:geometry}). The binding constraint is
vocabulary projection. The legal words realize only a sparse subset of the
directions the geometry offers, and the gap between the synthetic upper bound and
the embedding-rank image is the set of subsets whose separating direction no word
names (6.2, 6.3). Tightening the vocabulary toward more natural words widens that
gap monotonically.

A second wall stands behind the first. Even the separations a legal clue does
realize are often decoder artifacts rather than meanings: a projection-margin
maximizer exploits how one ranking algorithm orders words, and such clues do not
survive contact with a pragmatic receiver (6.4). Decoder-recoverability and
communicability are different properties, and the difference is large in exactly
the high-count, low-margin regime where embedding similarity is most tempting to
trust. What does carry to a pragmatic receiver is clean concept separation on the
whole board: when the targets share a sharp public category and the board holds no
same-category distractor, transfer rises from zero to roughly a third even at
counts 7 through 9, with occasional exact nines; a single same-category distractor
collapses exact recovery while leaving partial signal intact, and count retains a
secondary effect within clean clusters (6.5). None of this depends on constructed
challenge distributions: on natural first-turn boards exact recovery at counts 4
through 9 was zero across 384 audits, and when the cluegiver could choose its own
move it retreated to counts 1 through 3, succeeding only there (6.6). A symbolic
public-concept model with no embedding in it reaches the same high-count collapse,
which argues against explaining the result only as a GloVe-geometry artifact
(6.7).

Two implications follow for practice. Embedding cosine similarity is an unsafe
proxy for ``this clue communicates this set,'' and the failure concentrates where
it would do the most damage, at high target counts and low separating margins.
Systems that generate clues, captions, or category labels by optimizing a
similarity decoder are optimizing a target their human receiver does not share.
The variable worth controlling in clue generation and in board or benchmark design
is concept separation first and target count second: count is a much weaker
predictor of difficulty once cluster availability is in view.
...
symbol drawn from a shared, finite vocabulary, the binding constraint is the
...
and geometry are permissive; the binding constraint is the projection from an
...
to full-game value, where counts, turn order, and risk interact, is a separate
problem for which the recoverability map is a prerequisite, and we leave it to
future work.
